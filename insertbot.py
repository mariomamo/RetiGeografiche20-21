...
class Bot:
    ...
    def start_bot(self):
        ...
        insert_handler = MessageHandler(Filters.text & (~Filters.command), self.__insertHandler)
        self.__dispatcher.add_handler(insert_handler)
        self.__dispatcher.add_handler(CallbackQueryHandler(self.__button))
    ...
    #             txt = "Inserisci l'url di: " + self.__userdata[id][0]
    ...
    def __button(self, update: Update, context: CallbackContext) -> None:
        query = update.callback_query
        # CallbackQueries need to be answered, even if no notification to the user is needed
        # Some clients may have trouble otherwise. See https://core.telegram.org/bots/api#callbackquery
        query.answer()
        self.__userdata[update.callback_query.message.chat.id].append(query.data)
        print(self.__userdata[update.callback_query.message.chat.id])
